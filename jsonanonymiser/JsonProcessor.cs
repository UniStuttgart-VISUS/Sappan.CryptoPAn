...
namespace Sappan.JsonAnyonmiser {
    ...
    internal sealed class JsonProcessor {
        ...
        /// <summary>
        /// Anonymise all configured fields in <paramref name="token"/>.
        /// </summary>
        /// <param name="token"></param>
        private void ProcessRecord(JObject token) {
            foreach (var p in this._configuration.DomainNameFields) {
                var t = token.SelectToken(p) as JValue;
                if (t != null) {
                    var value = (string) t.Value;
                    value = this._stringScrambler.ScrambleDomainName(value);
                    t.Replace(value);
                }
            }
            ...
            foreach (var p in this._configuration.StringFields) {
                var t = token.SelectToken(p) as JValue;
                if (t != null) {
                    var value = (string) t.Value;
                    value = this._stringScrambler.Scramble(value);
                    t.Replace(value);
                }
            }
        }
